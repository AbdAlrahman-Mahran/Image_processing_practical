...
class App2(ctk.CTk):
    ...
    def apply_hpf(self):
        ...
        self.show_processed_img(cv2.cvtColor(hpf_img, cv2.COLOR_GRAY2BGR))

    def apply_mean(self):
        ksize = int(self.kernel_val.get())
        mean_img = cv2.blur(self.original_img, (ksize, ksize))
        self.show_processed_img(mean_img)
    ...
    def apply_median(self):
        ...
        self.show_processed_img(median_img)

    def apply_erosion(self):
        ksize = int(self.kernel_val.get())
        kernel = np.ones((ksize, ksize), np.uint8)
        eroded_img = cv2.erode(self.original_img, kernel, iterations=1)
        self.show_processed_img(eroded_img)

    def apply_dilation(self):
        ksize = int(self.kernel_val.get())
        kernel = np.ones((ksize, ksize), np.uint8)
        dilation_img = cv2.dilate(self.original_img, kernel, iterations=1)
        self.show_processed_img(dilation_img)

    def apply_open(self):
        ksize = int(self.kernel_val.get())
        kernel = np.ones((ksize, ksize), np.uint8)
        open_img = cv2.morphologyEx(self.original_img, cv2.MORPH_OPEN, kernel)
        self.show_processed_img(open_img)

    def apply_close(self):
        ksize = int(self.kernel_val.get())
        kernel = np.ones((ksize, ksize), np.uint8)
        close_img = cv2.morphologyEx(self.original_img, cv2.MORPH_CLOSE, kernel)
        self.show_processed_img(close_img)
