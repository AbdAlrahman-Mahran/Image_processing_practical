...
class App1(ctk.CTk):
    ...
    def apply_hough(self):
        ...
        circles = cv2.HoughCircles(gray_img, cv2.HOUGH_GRADIENT, 1, 50, param1=50, param2=30, minRadius=23, maxRadius=41)
        circles = np.uint16(np.around(circles))
        if circles is not None:
            # hough_img = self.original_img.copy()
    ...
    def add_slider(self):
        ...
        self.tvalue_slider = ctk.CTkSlider(self.tvalue_slider_frame, from_=0, to=255, number_of_steps=255,
                                           state='normal',
                                           command=self.update_threshold_value, variable=self.threshold_value)
